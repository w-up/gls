	...
	<div id="busenter">
		...
		<div class="con-wrapper">
			<div class="bus_con">
				<div class="bus_list">
					<span>公司法人：</span>
					<input type="text" v-model="legal_person" name="" placeholder="请输入公司法人姓名" />
				</div>
				<div class="bus_list">
					<span>法人身份证：</span>
					<input type="text"  v-model="legal_identity" name="" placeholder="请输入法人身份证号码" />
				</div>
				<div class="bus_list">
					<span>银行账号：</span>
					<input type="text" v-model="bank_card" name="" placeholder="请输入银行账户" />
				</div>
				<div class="bus_list">
					<span>开户行：</span>
					<input type="text" v-model="bank_name" name="" placeholder="请输入开户行" />
				</div>
				<div class="bus_list">
					<span>推荐商邀请码：</span>
					<input type="text" v-model="invitation_code" name="" placeholder="请输入推荐商邀请码" />
				</div>
				<div class="bus_list">
					<span>行业选择：</span>
		...
		methods: {
			...
			busEnter() {
				...
				if (!legal_person || legal_person == null) {
					Toast('请输入法人姓名');
				} else if (!legal_identity || legal_identity == null) {
					Toast('请输入法人身份证号');
			...
			getIndustrylist() {
				let that = this;
				that.$http.post('Ckshop/industryList', {
				}).then(function(res) {
					console.log(res)
					if (res.data.code == 0) {
						that.industry = res.data.data;
						console.log(that.industry)
					}
				}).catch(function(err) {
					Toast({
						message: '网络连接失败',
